...
///
/// LOCAL PATCH (see native/vendor/README.md): pinned to "linux" instead of the
/// real target OS.
///
/// On Android the real value makes librespot advertise an Android user agent
/// and platform to Spotify, while this client authenticates with the desktop
/// "keymaster" client id. Spotify checks that the two agree, so the access point
/// handshake succeeds but every client-token and login5 request is then refused
/// with a generic BAD_REQUEST — exactly the failure the comment above warns
/// about. Presenting a consistent desktop identity avoids it.
