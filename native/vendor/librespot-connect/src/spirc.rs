...
pub struct Spirc {
    ...
    /// LOCAL PATCH: mirrors `ConnectState::is_active` for the handle's owner.
    ///
    /// Whether this device is the one the account is playing on is the question
    /// every caller has to answer before it does anything, and until now only
    /// the event loop could answer it. Reading the cluster instead is not the
    /// same question: after playback is transferred *to* this device the
    /// account can go on naming the previous one for a while, and it does, so a
...
impl SpircTask {
    ...
    async fn handle_load(
        ...
        match cmd.context {
            ...
            PlayContext::Tracks(tracks) => self.load_context_from_tracks(tracks, None)?,
            PlayContext::TracksIn {
                context_uri,
                tracks,
            } => self.load_context_from_tracks(tracks, Some(context_uri))?,
        }
    ...
    fn load_context_from_tracks(
        ...
        // LOCAL PATCH: the real context these tracks came from, when there is
        // one. See `PlayContext::TracksIn`.
        context_uri: Option<String>,
    ) -> Result<(), Error> {
        const WEB_API_URI: &str = "spotify:web-api";
        let uri = context_uri.unwrap_or_else(|| WEB_API_URI.to_string());
        let ctx = Context {
