...
pub(super) enum PlayContext {
    Uri(String),
    Tracks(Vec<String>),
    /// LOCAL PATCH: an explicit list of tracks, published under a real context.
    ///
    /// Upstream has the two apart: a context uri, where the order is Spotify's,
    /// or a list of tracks, which is published as `spotify:web-api` because
    /// there is nothing better to call it. Neither fits a queue that *is* a
    /// playlist but not in the playlist's order, which is what shuffling one or
    /// adding a track to it produces, and that is most queues.
    ///
    /// The consequence of calling such a queue `spotify:web-api` is not
    /// cosmetic: no other device can resolve that context, so handing playback
    /// to one gives it a queue that does not play, and handing it back arrives
    /// with a state whose current track cannot be found.
    TracksIn {
        context_uri: String,
        tracks: Vec<String>,
    },
...
impl LoadRequest {
    ...
    /// LOCAL PATCH: create a load request from a set of `tracks` that belong to
    /// a real context, keeping the caller's order and publishing the context.
    pub fn from_tracks_in(
        context_uri: String,
        tracks: Vec<String>,
        options: LoadRequestOptions,
    ) -> Self {
        Self {
            context: PlayContext::TracksIn {
                context_uri,
                tracks,
            },
            options,
