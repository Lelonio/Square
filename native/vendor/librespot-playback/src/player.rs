...
struct PlayerInternal {
    ...
    /// LOCAL PATCH: the track being dissolved out of, if there is one.
...
///
/// A track that is on its way out is no longer the player's state: the state
/// holds the track coming in. Its decoder is kept here instead and read for as
/// long as the fade lasts, then dropped, which closes the stream behind it.
struct FadeOut {
    decoder: Decoder,
    /// The gain the outgoing track was being played at.
    ///
    /// Its own factor, not the incoming track's: normalisation exists so that
    ...
    /// Decoded samples not yet used, since a packet rarely lands exactly on the
    /// boundary of what the incoming packet needs.
    ...
    fn poll(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<()> {
        ...
        loop {
            ...
            } = self.state
            {
                ...
                // LOCAL PATCH: ask for the next track early, so it is already
                // playing while this one fades out.
                //
                // The end of a track is announced a crossfade's length before
                // it arrives. Whoever owns the queue answers with a load, the
                // usual way, and that load is what starts the fade: the decoder
                // still reading this track is handed to `fade_out` instead of
...
impl PlayerInternal {
    ...
    fn crossfade_samples(&self) -> usize {
        ...
        let samples = ms * SAMPLE_RATE as usize / 1000 * NUM_CHANNELS as usize;
        // Whole frames only: half a frame would swap the channels over for the
        // rest of the fade.
        samples - samples % NUM_CHANNELS as usize
    }
    ...
    ///
    /// Equal power, not a straight line: two straight ramps summed dip in the
    /// middle, where both tracks are at half, while cos and sin square to one
    /// and hold the loudness steady across the whole overlap.
    ///
        ...
        while offset + channels <= data.len() && fade.done < fade.total {
            ...
            while fade.spare.len() < channels && !fade.drained {
                match fade.decoder.next_packet() {
                    ...
                    // Ran out before the fade did: the incoming track goes on
    ...
    fn handle_player_stop(&mut self) {
        // LOCAL PATCH: nothing is being faded into any more.
        self.fade_out = None;
        match self.state {
            PlayerState::Playing {
                ref track_id,
    ...
    fn handle_packet(
        ...
        match packet {
            Some((_, mut packet)) => {
                ...
                // LOCAL PATCH: the outgoing track, mixed under this one.
                //
                // Here rather than earlier because both sides have to be at the
                // level they would have played at on their own: normalisation
                // and volume are applied above, and a fade that ran before them
                // would be a fade between two tracks nobody is hearing yet.
